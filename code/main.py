...
def get_img_contour_thresh(img):
    ...
    contours = [cnt for cnt in contours if cv2.contourArea(cnt) > 1000]  # Adjust the area threshold as needed
...
def text_mode(cam):
    ...
    word = ""
    count_same_frame = 0
    keypress = 0  # Initialize keypress to avoid UnboundLocalError
    while True:
        ret, img = cam.read()
        if not ret:
            print("Error: Could not read frame from camera.")
...
def recognize():
    ...
    if not cam.isOpened():
        # Try camera index 1 if index 0 is not available
