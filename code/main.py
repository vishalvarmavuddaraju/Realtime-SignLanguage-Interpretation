...
# ========== Backprojection and Thresholding ==========
...
def text_mode(cam):
    ...
    while True:
        ...
        voice_status = "On" if is_voice_on else "Off"
        cv2.putText(blackboard, f"Voice: {voice_status}", (450, 440), cv2.FONT_HERSHEY_TRIPLEX, 1, (255, 127, 0))
...
def recognize():
    ...
    keras_predict(model, np.zeros((50, 50), dtype=np.uint8))  # warm-up
    text_mode(cam)
    cam.release()
